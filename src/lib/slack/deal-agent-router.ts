...
export async function tryHandleWithDealAgent(opts: {
  ...
  const { userId, text, client, channel, threadTs } = opts;
  ...
  try {
    ...
    console.log(`[slack→deal-agent] user=${userId} matched=${matchedDeal} text=${cleaned.substring(0, 120)}`);
    ...
    const result = await runDealAgent({ userId, userMessage: cleaned });
    const slackReply = markdownToSlack(result.reply);
    await sendSlackMessage(client, channel, slackReply, threadTs);

    console.log(
      `[slack→deal-agent] user=${userId} turns=${result.turns} tools=${result.trace.length}${result.hitTurnCap ? " hitTurnCap" : ""}`
    );
    return true;
  } catch (err) {
