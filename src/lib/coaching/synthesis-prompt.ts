...
ALL the priorities that were agreed for the coming cycle, ordered by priority — exhaustive, never truncated to a count. If ten things were agreed as priorities, list ten. For each: the concrete action, the owner (default: the founder), and the success signal that says it's done. Be specific enough that a stranger reading this could pick up the work. No "continue working on X" — name the next move. (Don't pad, either — only things actually agreed as priorities belong here.)
