...
export const MIKEY_HEALTH_LEVELS = [
  { value: "excellent", label: "Excellent", color: "bg-green-100 text-green-700", emoji: "🟢" },
  { value: "good", label: "Good", color: "bg-emerald-100 text-emerald-700", emoji: "🟢" },
  { value: "fair", label: "Fair", color: "bg-amber-100 text-amber-700", emoji: "🟡" },
  { value: "poor", label: "Poor", color: "bg-red-100 text-red-700", emoji: "🔴" },
] as const;

export function getHealthInfo(value: string | null | undefined) {
  if (!value) return null;
  return MIKEY_HEALTH_LEVELS.find((h) => h.value === value) || null;
}
