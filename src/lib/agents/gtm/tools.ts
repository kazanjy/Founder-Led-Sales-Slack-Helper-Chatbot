      ...
      return {
        ...
        nudge:
          result.source === "pdl"
            ...
            : null,
        presentation:
          "Lead with the verdict headline, then the timeline showing what each company WAS during their tenure. Then the flags, GREEN FIRST and then red — lead with what's working before what to worry about. Both lists are already ordered by severity; show the high-severity ones with their claim, their evidence, and, on red flags, their innocentExplanation, which is not optional. Mark any flag whose confidence is 'possible' as approximate. If report.discountedFlags is non-empty, add a short 'considered and discounted' note listing them with the suppressedBy reason — showing the work is the point. Then 2-3 interview probes, then what couldn't be verified. Do NOT add, drop, or reword a flag: the list is deterministic. Do not invent a numeric score. If `gradedAgainst.hiringProfile` is true, say you graded against their AE Hiring Profile and never claim one was missing — and when `gradedAgainst.hiringProfileAccount` is present, name that account explicitly, for example: graded against Distill's AE Hiring Profile, so a profile from the wrong org is obvious on sight; only if it is false may you say no profile was authored. Finally, if `nudge` is non-null, end with it in your own words — the founder got the thinner read and should know a résumé would sharpen it. If `nudge` is null they already gave us everything, so say nothing about inputs.",
      };
    } catch (err) {
