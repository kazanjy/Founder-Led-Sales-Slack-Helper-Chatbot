...
export default function HiringProfileHistory() {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [versions, setVersions] = useState<VersionSummary[]>([]);

  useEffect(() => {
    document.title = "AE Hiring Profile History - Mikey";
  }, []);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const response = await fetch("/api/hiring-profile/history");
        if (response.ok) {
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">AE Hiring Profile History</h1>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">Generate your first AE Hiring Profile to see it here.</p>
            <Link
          ...
          <div className="space-y-3">
            {versions.map((v, index) => (
              <Link
                key={v.id}
                href={`/hiring-profile?version=${v.id}`}
