...
export default function CallRecapHistory() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const response = await fetch("/api/call-recap/history");
        if (response.ok) {
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Call Recap Email History</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">{versions.length} version{versions.length !== 1 ? "s" : ""}</p>
            </div>
          </div>
          <Link
            href="/call-recap/new"
            className="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-lg hover:from-purple-700 hover:to-blue-700 transition-all font-medium text-sm shadow-sm flex items-center gap-2"
          >
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">Generate your first Call Recap Email to see it here.</p>
            <Link
              href="/call-recap/new"
          ...
          <div className="space-y-3">
            {versions.map((v, index) => (
              <Link
                ...
                className="group block bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 hover:border-purple-300 hover:shadow-md transition-all"
              >
                    ...
                    <div className="flex items-center gap-2">
                      {editingId === v.id ? (
                        <input
                          ...
                          autoFocus
                          className="font-semibold text-gray-900 dark:text-gray-100 bg-white dark:bg-gray-800 border border-purple-300 rounded px-2 py-0.5 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 w-full"
                        />
                      ) : (
