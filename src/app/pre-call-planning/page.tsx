...
function PreCallPlanningContent() {
  ...
  const handleDelete = async () => {
    ...
    const confirmed = await showConfirm({
      title: "Delete Pre-Call Planning",
      message: "Are you sure you want to delete this Pre-Call Planning? This cannot be undone.",
      variant: "danger",
      confirmLabel: "Delete",
    });
    if (!confirmed) return;
    setDeleting(true);
    try {
      const res = await fetch(`/api/pre-call-planning/versions/${version.id}`, { method: "DELETE" });
      if (res.ok) {
        const data = await res.json();
        if (data.hasRemaining) {
          window.location.href = "/pre-call-planning";
        } else {
          router.push("/pre-call-planning");
        }
      } else {
        setDeleting(false);
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {isEditing ? (
                ...
                <>
                  <ChatAboutButton
                    title="Chat About Pre-Call Checklist"
