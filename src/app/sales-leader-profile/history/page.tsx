...
export default function SalesLeaderProfileHistory() {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [versions, setVersions] = useState<VersionSummary[]>([]);

  useEffect(() => {
    document.title = "Sales Leader Hiring Profile History - Mikey";
  }, []);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const response = await fetch("/api/sales-leader-profile/history");
        if (response.ok) {
