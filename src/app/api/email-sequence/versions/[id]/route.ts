...
export async function PATCH(
  ...
  try {
    ...
    const existingVersion = await prisma.emailSequenceVersion.findFirst({
    ...
    const updatedVersion = await prisma.emailSequenceVersion.update({
      where: { id },
      data: { content },
    });
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.emailSequenceVersion.delete({ where: { id } });

    const newLatest = await prisma.emailSequenceVersion.findFirst({
    ...
    await prisma.gtmVariable.upsert({
      where: {
        userId_mergeField: { userId: user.id, mergeField: "EMAIL_SEQUENCE" },
      },
