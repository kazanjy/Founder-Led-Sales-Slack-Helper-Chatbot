...
export async function DELETE(
  ...
  try {
    ...
    await prisma.preCallPlanningVersion.delete({ where: { id } });

    const newLatest = await prisma.preCallPlanningVersion.findFirst({
    ...
    await prisma.gtmVariable.upsert({
      where: {
        userId_mergeField: { userId: user.id, mergeField: "PRE_CALL_PLANNING" },
      },
