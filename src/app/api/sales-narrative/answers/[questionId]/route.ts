...
export async function GET(
  ...
  try {
    ...
    const question = await prisma.salesNarrativeQuestion.findUnique({
      where: { id: questionId },
    });
    ...
    const answers = await prisma.salesNarrativeAnswer.findMany({
      where: {
        userId: user.id,
        questionId,
      },
...
export async function POST(
  ...
  try {
    ...
    const newAnswer = await prisma.salesNarrativeAnswer.create({
      data: {
        userId: user.id,
        questionId,
        answer: answer.trim(),
    ...
    const nextQuestion = await prisma.salesNarrativeQuestion.findFirst({
      where: {
