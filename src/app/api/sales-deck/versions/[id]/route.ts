  ...
  try {
    ...
    return NextResponse.json({
      version: {
        ...
        sourcePdfName: version.sourcePdfName,
...
export async function PATCH(
  ...
  try {
    ...
    const existingVersion = await prisma.salesDeckVersion.findFirst({
    ...
    const updatedVersion = await prisma.salesDeckVersion.update({
      where: { id },
      data: { content },
    });
...
export async function DELETE(
  ...
  try {
    ...
    await prisma.salesDeckVersion.delete({ where: { id } });

    const newLatest = await prisma.salesDeckVersion.findFirst({
    ...
    await prisma.gtmVariable.upsert({
      where: {
        userId_mergeField: { userId: user.id, mergeField: "SALES_DECK" },
      },
