...
// GET - Get the latest sales leader profile version
export async function GET() {
  try {
    ...
    const latestVersion = await prisma.salesLeaderProfileVersion.findFirst({
    ...
    console.error("Error fetching latest sales leader profile:", error);
    return NextResponse.json(
      { error: "Failed to fetch latest sales leader profile" },
      { status: 500 }
    );
  }
}
