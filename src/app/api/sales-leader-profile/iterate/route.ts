...
// POST - Iterate on an existing sales leader profile version, streaming the result
export async function POST(request: NextRequest) {
  try {
    ...
    const version = await prisma.salesLeaderProfileVersion.findFirst({
    ...
    const prompt = `You are an expert sales hiring consultant. You previously generated a Sales Leader Hiring Profile report, and the founder has provided feedback to revise it.
...
Revise the Sales Leader Hiring Profile based on the feedback above. Keep what works well, improve or change what was called out.
...
Be specific and actionable. Output ONLY the revised markdown report, no JSON wrapping.`;
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          const updatedVersion = await prisma.salesLeaderProfileVersion.update({
            where: { id: versionId },
            data: {
              content: fullContent,
          ...
          console.error("[sales-leader-profile/iterate] Error:", error);
    ...
    console.error("[sales-leader-profile/iterate] Setup error:", error);
