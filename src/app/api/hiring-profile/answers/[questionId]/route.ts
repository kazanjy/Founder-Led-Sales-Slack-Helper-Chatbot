...
export async function GET(
  ...
  try {
    ...
    const question = await prisma.hiringProfileQuestion.findUnique({
      where: { id: questionId },
    });
    ...
    const answers = await prisma.hiringProfileAnswer.findMany({
      where: {
        userId: user.id,
        questionId,
      },
...
export async function POST(
  ...
  try {
    ...
    const newAnswer = await prisma.hiringProfileAnswer.create({
      data: {
        userId: user.id,
        questionId,
        answer: answer.trim(),
    ...
    const nextQuestion = await prisma.hiringProfileQuestion.findFirst({
      where: {
