...
interface ColdCallScriptVersion {
  id: string;
  content: string;
  orgPersona: string;
  humanPersona: string;
...
export default function ColdCallScriptPage() {
  return (
    <Suspense fallback={
      ...
      <ColdCallScriptContent />
    </Suspense>
  );
}

function ColdCallScriptContent() {
  ...
  const [version, setVersion] = useState<ColdCallScriptVersion | null>(null);
  ...
  const [scriptType, setScriptType] = useState<"outbound" | "inbound">("outbound");
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        let url = "/api/cold-call-script/latest";
        if (versionId) {
          url = `/api/cold-call-script/versions/${versionId}`;
        }
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true);
    try {
      const response = await fetch("/api/cold-call-script/generate", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        await showAlert({
          title: "Error",
          message: data.error || "Failed to generate cold call script",
          variant: "danger",
        });
        return;
      }
      ...
      await showAlert({
        title: "Error",
        message: "Failed to generate cold call script. Please try again.",
        variant: "danger",
      });
    } finally {
      setGenerating(false);
  ...
  const handleRegenerate = () => {
    if (version) {
      ...
      setScriptType(version.scriptType as "outbound" | "inbound");
  ...
  const handleImportPDF = async (file: File) => {
    ...
    try {
      ...
      formData.append("appletType", "coldCallScript");
  ...
  const handleDelete = async () => {
    ...
    const confirmed = await showConfirm({
      title: "Delete Cold Call Script",
      message: "Are you sure you want to delete this Cold Call Script? This cannot be undone.",
      variant: "danger",
      confirmLabel: "Delete",
    });
    if (!confirmed) return;
    setDeleting(true);
    try {
      const res = await fetch(`/api/cold-call-script/versions/${version.id}`, { method: "DELETE" });
      if (res.ok) {
        const data = await res.json();
        if (data.hasRemaining) {
          window.location.href = "/call-scripts";
        } else {
          router.push("/call-scripts");
        }
      } else {
        setDeleting(false);
  ...
  const handleClone = async () => {
    ...
    try {
      const res = await fetch("/api/documents/clone", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ documentType: "coldCallScript", documentId: version.id }),
      });
      if (res.ok) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const response = await fetch(`/api/cold-call-script/versions/${version.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <GeneratingOverlay
        ...
        messages={[
          ...
          "Crafting pattern interrupt opener",
          "Writing elevator pitch",
          "Building objection handles",
          "Designing close for meeting",
          ...
          <div className="flex flex-col gap-3 mb-6">
            ...
            <div className="flex items-center gap-3">
              <div className="px-3 py-2 bg-orange-50 text-orange-800 text-sm rounded-lg border border-orange-100">
                <span className="font-semibold text-orange-600">Type:</span> {scriptTypeLabel(version.scriptType)}
              </div>
            </div>
