...
export default function AdCreatorHistoryPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [versions, setVersions] = useState<VersionSummary[]>([]);

  useEffect(() => {
    document.title = "Ad Creator History - Mikey";
  }, []);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const response = await fetch("/api/ad-creator/history");
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-5xl mx-auto px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Ad Creator History</h1>
        ...
        {versions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Generate your first ad concepts to see history here.
            </p>
            <Link
              href="/ad-creator"
          ...
          <div className="space-y-4">
            {versions.map((version, index) => (
              <Link
                key={version.id}
                href={`/ad-creator?version=${version.id}`}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mb-1">
                      {(version.platforms || []).map(p => (
                        <span key={p} className="px-1.5 py-0.5 bg-gray-100 text-gray-600 dark:text-gray-300 text-xs rounded">
                          {platformLabels[p] || p}
                        </span>
                      ))}
                    </div>
                    ...
                    {version.specialNotes && (
                      <p className="text-xs text-gray-500 dark:text-gray-400 mb-1 line-clamp-1">
                        <span className="font-medium text-gray-600 dark:text-gray-300">Notes:</span> {version.specialNotes}
                      </p>
                    )}
                    {version.iterationNotes && (
                      <p className="text-xs text-purple-500 mb-1 line-clamp-1">
                        <span className="font-medium text-purple-600">Iteration:</span> {version.iterationNotes}
                      </p>
                    )}
