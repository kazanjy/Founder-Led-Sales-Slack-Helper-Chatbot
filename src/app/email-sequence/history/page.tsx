...
export default function EmailSequenceHistoryPage() {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [versions, setVersions] = useState<VersionSummary[]>([]);

  useEffect(() => {
    document.title = "Email Sequence History - Mikey";
  }, []);
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const response = await fetch("/api/email-sequence/history");
      ...
      <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-5xl mx-auto px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Email Sequence History</h1>
        ...
        {versions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-6">
              Generate your first email sequence to see history here.
            </p>
            <Link
              href="/email-sequence"
          ...
          <div className="space-y-4">
            {versions.map((version, index) => (
              <Link
                key={version.id}
                href={`/email-sequence?version=${version.id}`}
