...
const gtmMaturityItems: NavItem[] = [
  { href: "/assessment/bulk", label: "📊 GTM Maturity Assessment", statusKey: "assessment" },
  { href: "/sales-readiness", label: "✅ GTM Readiness Progression", statusKey: "salesReadiness" },
];
...
const playbookItems: NavItem[] = [
  ...
  { href: "/sales-asset-library", label: "📚 Sales Asset Library", statusKey: "salesAssetLibrary" },
];
  ...
  useEffect(() => {
    async function fetchStatus() {
      try {
        ...
        const [narrativeRes, icpRes, discoveryRes, checklistRes, planningRes, researchRes, assessmentRes, emailSeqRes, linkedInSeqRes, callReviewRes, coldCallRes, salesDeckRes, salesMetricsRes, objectionLibraryRes, socialContentRes, assetLibraryRes] = await Promise.all([
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        ...
        {mobileMenuOpen && (
          <div ref={mobileMenuRef} className="md:hidden absolute left-0 right-0 top-auto bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 shadow-lg z-50 max-h-[70vh] overflow-y-auto">
            <div className="py-2">
              ...
              {/* GTM Maturity */}
              <div className="border-t border-gray-100 dark:border-gray-700 mt-1 pt-1">
                <div className="px-4 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">GTM Maturity</div>
                {gtmMaturityItems.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    onClick={closeMenus}
        ...
        <div className="hidden md:flex items-center gap-1 -mb-px flex-wrap">
          ...
          <div className="relative" ref={gtmMaturityDropdownRef}>
            ...
            >
              📋 GTM Maturity
              <svg className={`w-3.5 h-3.5 transition-transform ${gtmMaturityOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>

            {gtmMaturityOpen && (
              <div className="absolute top-full left-0 mt-px bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg py-1 z-50 min-w-[260px]">
                {gtmMaturityItems.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    onClick={() => setGtmMaturityOpen(false)}
